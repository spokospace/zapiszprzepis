...
const STORAGE_URL_PREFIX = '/storage/v1/object/public/recipe-images/'
...
async function main(): Promise<void> {
  ...
  const { data: recipes, error } = await supabase
    .from('recipes')
    .select('id, source_url, user_id, source_type, image_url')
  ...
  if (!recipes || recipes.length === 0) {
    console.log('✓ No recipes to scan.')
    return
  }
  ...
  const targets = recipes.filter((r) => {
    ...
    return !r.image_url.includes(STORAGE_URL_PREFIX)
  })

  if (targets.length === 0) {
    console.log('✓ Nothing to archive — all recipes already point at Supabase Storage.')
    return
  }

  console.log(`Found ${targets.length} recipe(s) with non-archived images.`)
  ...
  for (const r of targets) {
    ...
    console.log(`  ✓ Triggered archive for recipe ${r.id} (${r.source_url})`)
  }
