...
- [x] 4.1 TypeScript: `npx tsc --noEmit` — c51c78a
- [x] 4.2 ESLint: `npm run lint` — c51c78a
- [x] 4.3 `npm run build` przechodzi — c51c78a
- [x] 4.4 Istnieją wymagane pliki UI (`login/page.tsx`, `login/actions.ts`, `auth/callback/route.ts`, `(actions)/sign-out.ts`, `lib/site-url.ts`) — c51c78a

#### Ręczne

- [x] 4.5 `npm run dev` → `/` → redirect na `/login` → widać polski formularz — c51c78a
- [x] 4.6 Submit emaila → `/login?sent=1&email=...` → polski komunikat sukcesu — c51c78a
- [x] 4.7 Magic-link przychodzi do skrzynki w < 1 min — c51c78a
- [x] 4.8 Klik linku → `/auth/callback?code=...` → redirect na `/` → "Zalogowano jako <email>" — c51c78a
- [x] 4.9 Próba wejścia na `/login` ze ważną sesją → redirect na `/` — c51c78a
- [x] 4.10 "Wyloguj się" → `/login`; ponowna próba wejścia na `/` redirectuje na `/login` — c51c78a
- [x] 4.11 Zużyty link → `/login?error=used` → polski komunikat — c51c78a
- [x] 4.12 Brak `code` w callbacku → `/login?error=invalid` — c51c78a
- [x] 4.13 Bez internetu submit → `/login?error=unknown` (nie crash) — c51c78a
