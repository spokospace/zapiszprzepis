...
- [x] 3.1 `supabase/config.toml` istnieje
- [x] 3.2 `supabase/migrations/` zawiera dokładnie 1 plik `*_init_auth_helpers.sql`
- [x] 3.3 `npx supabase migration list --linked` pokazuje migrację jako applied (Remote ✓)

#### Ręczne

- [x] 3.4 Supabase Dashboard → Database → Extensions: `pg_trgm` jest `Enabled`
- [x] 3.5 Database → Functions: `public.current_user_id` istnieje (`STABLE SECURITY DEFINER`)
- [x] 3.6 SQL Editor: `SELECT public.current_user_id()` zwraca NULL bez sesji
