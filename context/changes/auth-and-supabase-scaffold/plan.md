...
- [x] 1.1 `package.json` zawiera `@supabase/ssr`, `@supabase/supabase-js`, `supabase`, `tsx`
- [x] 1.2 `.env.local.example` istnieje i ma 3 wymagane klucze
- [x] 1.3 `.env.local` jest ignorowane przez git
- [x] 1.4 `npm run dev` startuje bez błędów importu

#### Ręczne

- [x] 1.5 Projekt Supabase widoczny w dashboardzie pod nazwą `zapiszprzepis`, region `eu-central-1`
- [x] 1.6 Redirect URLs allowlist zawiera 3 wpisy (localhost + 2× Vercel)
- [x] 1.7 `.env.local` lokalnie zawiera prawdziwe wartości
- [x] 1.8 Vercel ma 3 zmienne ustawione dla Production + Preview

### Faza 2: Klienty SSR + proxy

#### Automatyczne

- [ ] 2.1 TypeScript się kompiluje: `npx tsc --noEmit`
- [ ] 2.2 ESLint przechodzi: `npm run lint`
- [ ] 2.3 `proxy.ts` istnieje na root projektu
- [ ] 2.4 `src/lib/supabase/{server,client,proxy}.ts` istnieją
- [ ] 2.5 `npm run build` przechodzi

#### Ręczne

- [ ] 2.6 `npm run dev` → wejście na `/` przekierowuje na `/login` (307)
- [ ] 2.7 Devtools Network potwierdza redirect 307 z `Location: /login`
- [ ] 2.8 Próba wejścia na `/login` bez sesji nie powoduje pętli redirect

### Faza 3: Schema baseline

#### Automatyczne

- [ ] 3.1 `supabase/config.toml` istnieje
- [ ] 3.2 `supabase/migrations/` zawiera dokładnie 1 plik `*_init_auth_helpers.sql`
- [ ] 3.3 `npx supabase migration list --linked` pokazuje migrację jako applied (Remote ✓)

#### Ręczne

- [ ] 3.4 Supabase Dashboard → Database → Extensions: `pg_trgm` jest `Enabled`
- [ ] 3.5 Database → Functions: `public.current_user_id` istnieje (`STABLE SECURITY DEFINER`)
- [ ] 3.6 SQL Editor: `SELECT public.current_user_id()` zwraca NULL bez sesji
...
- [ ] 4.4 Istnieją wymagane pliki UI (`login/page.tsx`, `login/actions.ts`, `auth/callback/route.ts`, `(actions)/sign-out.ts`, `lib/site-url.ts`)

#### Ręczne

- [ ] 4.5 `npm run dev` → `/` → redirect na `/login` → widać polski formularz
- [ ] 4.6 Submit emaila → `/login?sent=1&email=...` → polski komunikat sukcesu
- [ ] 4.7 Magic-link przychodzi do skrzynki w < 1 min
- [ ] 4.8 Klik linku → `/auth/callback?code=...` → redirect na `/` → "Zalogowano jako <email>"
- [ ] 4.9 Próba wejścia na `/login` ze ważną sesją → redirect na `/`
- [ ] 4.10 "Wyloguj się" → `/login`; ponowna próba wejścia na `/` redirectuje na `/login`
- [ ] 4.11 Zużyty link → `/login?error=used` → polski komunikat
- [ ] 4.12 Brak `code` w callbacku → `/login?error=invalid`
- [ ] 4.13 Bez internetu submit → `/login?error=unknown` (nie crash)
